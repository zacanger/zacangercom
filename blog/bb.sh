...
global_variables() {
    ...
    template_archive_index_page="up to the blog"
    ...
    template_twitter_button="tweet it?"
...
create_includes() {
    ...
    if [[ -f "$header_file" ]]; then cp "$header_file" .header.html
        ...
        echo '<html lang="en"><head>' >> ".header.html"
        echo '<meta charset="utf-8">' >> ".header.html"
        echo '<meta name="viewport" content="width=device-width, initial-scale=1.0">' >> ".header.html"
        echo '<script type="text/javascript" src="./prism.js"></script>' >> ".header.html"
        echo '<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/jquery/2.2.0/jquery.min.js"></script>' >> ".header.html"
    ...
    if [[ -f "$footer_file" ]]; then cp "$footer_file" .footer.html
        ...
        echo 'written in <a href="https://daringfireball.net/projects/markdown/">markdown</a> and generated with a <a href="http://tiswww.case.edu/php/chet/bash/bashtop.html">shell script</a></div>' >> ".footer.html"
        echo '<a href="http://zacanger.com/me">zac anger?</a>' >> ".footer.html"
    fi
}
