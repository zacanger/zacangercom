// set shouldLog to true to turn on console.logging, or false to turn it all off
var
  shouldLog       = false
...
, version         = null
, GITHUB_CALLBACK = {}
, _CUSTOMSIZE_CONFIGURATION_ = _CUSTOMSIZE_CONFIGURATION_ || {}
, username
, h
, w
, token = {
...
GITHUB_CALLBACK['_users_' + username] = function (resp) {
  ...
  $('#tpl-wrap .forkcard').css({height : h})
...
GITHUB_CALLBACK['_users_' + username + '_repos'] = function (resp) {
  ...
  if (resp.meta.Link && resp.meta.Link[0][1].rel === 'next') {
    getScript(resp.meta.Link[0][0], {full : true})
  } else {
    renderAllRepos()
    renderRepos()
    // getVersion()
...
GITHUB_CALLBACK['_contribute_calendar_data'] = function (data) {
  ...
  while (length--) {
    var
      col       = Math.floor(length / 12) + 1
    , row       = (length % 12) + 1
    , className = 'c' + (data[length][1] < 15 ? data[length][1] : 'top')
    $('#contribute tr:nth-child(' + row + ') td:nth-child(' + col + ')')
    .attr('title', data[length][0])
    .addClass(className)
  }
}
...
var renderAllRepos = function () {
  var
    container   = $('.all-repos .panel-body')
  , currInitial = null
  , prevInitial = null
...
var renderRepos = function () {
  ...
  if (repoFork.length) {
    ...
    $.each(repoFork, function (index, r) {
      node
      .find('.repo-name')
      .append('<a target="_blank" class="name html_url" href="' + r.html_url + '">' + r.name + '</a>')
    })
